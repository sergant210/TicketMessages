...
//Переписка возможна только для зарегистрированных пользователей
if (!$modx->user->isAuthenticated($modx->context->get('key'))) return '';
...
if (!empty($rows) && is_array($rows)) {
	foreach ($rows as $k => $row) {
		$suffix = $row['id'] > $modx->user->id ? $modx->user->id.'/'.$row['id'] : $row['id'].'/'.$modx->user->id;
		$row['thread'] = $threadname = 'message'.$suffix;
		$thread = $pdoFetch->getObject('TicketThread',array('name'=>$threadname),array('select'=>'closed,deleted,comments'));
		$row['thread_closed'] = 'false';
		if (!$thread['deleted']) {
			if ($thread['closed']) {
				$row['thread_closed_class'] = 'thread-closed';
				$row['thread_closed'] = 'true';
			}
			$row['messages'] = $thread['comments'];
